...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const [dailySummary, userProfile] = await Promise.all([
  ...
  const calorieProgress = summary
    ? Math.min((summary.total_kcal / calorieGoal) * 100, 100)
    : 0;
  const remainingCalories = Math.max(
    calorieGoal - (summary?.total_kcal || 0),
    0
  );
      ...
      <div className="mx-auto max-w-md">
        ...
        <div className="bg-emerald-700 text-white p-6 rounded-b-3xl shadow-lg">
          <div className="text-center space-y-2">
            <h1 className="text-2xl font-bold text-white">CalKal</h1>
        ...
        <div className="p-4 space-y-6">
              ...
              <div className="text-center space-y-4">
                <div className="relative">
                  ...
                  <div className="text-sm text-muted-foreground">
                    จาก {calorieGoal.toLocaleString()} แคลอรี่
                  </div>
                </div>
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">
                    เหลือ {remainingCalories.toLocaleString()} แคล
                  </span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Link href="/capture">
              <Button
                className="w-full h-20 flex-col gap-2 bg-emerald-600 hover:bg-emerald-700"
                size="lg"
              >
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">
                      {Math.round(profile.bmr)} แคล/วัน
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-muted-foreground">TDEE</span>
                    <span className="font-medium">
                      {Math.round(profile.tdee)} แคล/วัน
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-bold text-emerald-600">
                      {profile.daily_kcal_goal} แคล/วัน
                    </span>
                  </div>
                </div>
              </CardContent>
